  ...
  (
    ...
    async function selectOption(id: string) {
      ...
      if (currentQuestion) {
        ...
        const attendeePoints =
          ((score * millisecondsLeft) /
            (Number(currentQuestion?.duration) * 1000)) *
    ...
    const currentParticipants = useMemo(() => {
      if (Array.isArray(actualQuiz?.quizParticipants)) {
        const filteredParticipants = actualQuiz?.quizParticipants?.filter(
          (v) => {
      ...
      <>
        <div
          className={cn(
            ...
            !isLeftBox &&
              !isRightBox &&
              "col-span-full rounded-xl max-w-4xl h-[92vh] sm:h-[78vh] inset-0 absolute m-auto",
